...
def browsefunc():
    global my_file
    filename = askopenfilename(title='Select A File',filetypes=(('text files','*.txt'),('all files','*')))
    path=filename

    key=Fernet.generate_key()
    print('Key= ',key)
    file=open('c:\doclocker\key.key','wb')
    file.write(key)#the key will be in type bytes
    file.close()
    ...
    def decrypt():
        global my_file
        output_file='c:\doclocker\encryptedfile.txt'
        with open(output_file,'wb') as f:
            f.write(encrypted)
        with open(output_file,'rb') as f:
            data=f.read()
        print('encrypted content of the file is:', data.decode())
        e=Fernet(key)
        decrypted_file=e.decrypt(data)
        print('the decrypted content of the file is:',decrypted_file.decode())
              

    #asking the user whether he has to decrypt the program or just signout
    
    btn1t1=ttk.Button(text='Decrypt', command=decrypt)
    btn1t1.pack()
